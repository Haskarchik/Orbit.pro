    ...
    return (
        <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                <div className="glass-card border-orange-500/20 bg-orange-500/5 p-8 flex items-center gap-8 group hover:bg-orange-500/10 transition-all">
                    <div className="w-14 h-14 bg-orange-600/10 text-orange-500 rounded-2xl flex items-center justify-center border border-orange-600/20 group-hover:scale-110 transition-transform">
                        <ShieldCheck size={32} />
                    </div>
                    <div>
                        <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2">{dict.discipline}</div>
                        <div className="text-3xl font-black italic text-white leading-none">{disciplineLevel}%</div>
                    </div>
                </div>
                {/* ... other stats ... */}
                <div className="glass-card border-emerald-500/20 bg-emerald-500/5 p-8 flex items-center gap-8 group hover:bg-emerald-500/10 transition-all">
                    <div className="w-14 h-14 bg-emerald-600/10 text-emerald-500 rounded-2xl flex items-center justify-center border border-emerald-600/20 group-hover:scale-110 transition-transform">
                        <Wallet size={32} />
                    </div>
                    <div>
                        <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2">{dict.totalBalance}</div>
                        <div className="text-3xl font-black italic text-white leading-none">{currencySymbol}{finances.reduce((acc, f) => f.type === 'income' ? acc + f.amount : acc - f.amount, 0).toLocaleString()}</div>
                    </div>
                </div>
                <div className="glass-card border-cyan-500/20 bg-cyan-500/5 p-8 flex items-center gap-8 group hover:bg-cyan-500/10 transition-all">
                    <div className="w-14 h-14 bg-cyan-600/10 text-cyan-500 rounded-2xl flex items-center justify-center border border-cyan-600/20 group-hover:scale-110 transition-transform">
                        <TargetIcon size={32} />
                    </div>
                    <div>
                        <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2">{dict.savings}</div>
                        <div className="text-3xl font-black italic text-white leading-none">{savingsGoals.length} ACTIVE</div>
                    </div>
                </div>
                <div className="glass-card border-slate-800 bg-slate-900/40 p-8 flex items-center gap-8 group hover:bg-slate-800/80 transition-all">
                    <div className="w-14 h-14 bg-slate-800 text-slate-400 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
                        <BarChart3 size={32} />
                    </div>
                    <div>
                        <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-2">{dict.longestStreak}</div>
                        <div className="text-3xl font-black italic text-white leading-none">{Math.max(...habits.map(h => h.streak), 0)} {lang === 'ua' ? 'ДН.' : 'DAYS'}</div>
                    </div>
                </div>
            </div>
                ...
                <div className="col-span-12 lg:col-span-8 space-y-12">
                    {activeSubview === 'protocol' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10 animate-in slide-in-from-bottom-4 duration-700">
                             <div className="glass-card border-slate-800/50 bg-slate-900/10 p-8">
                                <div className="flex items-center justify-between mb-10">
                                    <h3 className="text-sm font-black italic text-white flex items-center gap-3 uppercase tracking-widest">
                                ...
                                <div className="h-[380px] overflow-y-auto pr-2 space-y-4 scrollbar-hide">
                                    {habits.map(h => (
                                        <div key={h.id} className="flex items-center justify-between p-5 bg-slate-950/40 border border-slate-800/30 rounded-3xl group hover:border-orange-500/30 transition-all cursor-pointer" onClick={() => handleToggle(h.id, new Date().getDate() - 1)}>
                            ...
                            <div className="glass-card p-8 border-slate-800">
                                <h3 className="text-sm font-black italic text-white mb-8 uppercase flex items-center gap-3 tracking-widest">
                                ...
                                <div className="space-y-6">
                                    {savingsGoals.slice(0, 3).map(goal => (
                                        <div key={goal.id} className="space-y-3">
                                            <div className="flex justify-between px-1"><span className="text-[10px] font-black text-slate-500 italic uppercase">{goal.name}</span><span className="text-[10px] font-black text-orange-500">{Math.round((goal.current/goal.target)*100)}%</span></div>
